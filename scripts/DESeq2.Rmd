...
head(m_v_c.sig)
sum(m_v_c.sig$log2FoldChange <0, na.rm=T) # 1 gene is downregulated in mid relative to control
...
heatmap <- pheatmap(mat, annotation_col=df, clustering_method = "average", scale="row", clustering_distance_rows="euclidean", show_rownames =TRUE,fontsize_row = 4, cluster_cols=T, cluster_rows=TRUE, show_colnames =TRUE, color = my_color_palette); heatmap #plot heatmap of all DEG by group
...
sum(h_v_c.sig$log2FoldChange <0, na.rm=T) # 249 genes are downregulated in high relative to control
sum(h_v_c.sig$log2FoldChange >0, na.rm=T) # 372 genes are upregulated in high relative to control
#plotCounts(DEG.int, gene="Pocillopora_acuta_HIv2___RNAseq.g3840.t1", intgroup="Treatment")
...
head(h_v_m.sig)
sum(h_v_m.sig$log2FoldChange <0, na.rm=T) # 263 genes are downregulated in high relative to mid
sum(h_v_m.sig$log2FoldChange >0, na.rm=T) # 329 genes are upregulated in high relative to mid
...
heatmap <- pheatmap(mat, annotation_col=df, clustering_method = "average", scale="row", clustering_distance_rows="euclidean", show_rownames =F,fontsize_row = 4, cluster_cols=T, cluster_rows=TRUE, show_colnames =F, color = my_color_palette, annotation_legend = F, annotation_names_col = F, annotation_colors = list(Treatment = c("Control" = my_colors[1], "Mid" = my_colors[2], "High" = my_colors[3]))); heatmap #plot heatmap of all DEG by group

ggsave("../output/DEG/heatmap_DEGs.pdf", heatmap, width = 8, height = 10)
ggsave("../output/DEG/heatmap_DEGs.png", heatmap, width = 8, height = 10)
```
