...
# Plot 
shared_down <- ggplot(down_filt, aes(x = Treatment, y = mean_log_count, fill = Treatment)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  # geom_errorbar(aes(ymin = mean_log_count - sd_count, ymax = mean_log_count + sd_count),
...
# Plot w/ error bars
shared_down_error <- ggplot(down_filt, aes(x = Treatment, y = mean_log_count, fill = Treatment)) +
  geom_bar(stat = "identity", position = "identity", color = "black") +
   geom_errorbar(aes(ymin = mean_log_count - sd_log_count, ymax = mean_log_count + sd_log_count),
                 width = 0.2,  # Adjust width as needed
                 position = position_dodge(width = 0.9)) + # Adjust dodge width as needed
  facet_wrap(~gene_id, scales = "free", nrow = 1) +
  labs(x = "", y = expression(bold(paste("log"[2], " Normalized Counts")))) +
  ...
  theme(axis.text = element_text(size = 20, color="black"),
        ...
        strip.text = element_text(size = 17, color = "black")); shared_down_error
ggsave("../output/functional_enrichment/Downregulated_shared_genes_WithErrorBars.pdf", shared_down_error, width = 20, height = 14)
ggsave("../output/functional_enrichment/Downregulated_shared_genes_WithErrorBars.png", shared_down_error, width = 20, height = 14)
```
...
# Plot 
shared_up <- ggplot(up_filt, aes(x = Treatment, y = mean_log_count, fill = Treatment)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  # geom_errorbar(aes(ymin = mean_log_count - sd_log_count, ymax = mean_log_count + sd_log_count),
  #               width = 0.2,  # Adjust width as needed
  #               position = position_dodge(width = 0.9)) + # Adjust dodge width as needed
...
# Plot w/ error bars
shared_up_error <- ggplot(up_filt, aes(x = Treatment, y = mean_log_count, fill = Treatment)) +
  geom_bar(stat = "identity", position = "identity", color = "black") +
   geom_errorbar(aes(ymin = mean_log_count - sd_log_count, ymax = mean_log_count + sd_log_count),
                 width = 0.2,  # Adjust width as needed
                 position = position_dodge(width = 0.9)) + # Adjust dodge width as needed
  ...
  theme(axis.text = element_text(size = 20, color="black"),
        axis.title=element_text(size=25, color="black", face="bold"),
        #axis.line = element_line(colour = 'black', size = 1),
        strip.text = element_text(size = 17, color = "black")); shared_up_error
ggsave("../output/functional_enrichment/Upregulated_shared_genes_WithErrorBars.pdf", shared_up_error, width = 15, height = 14)
ggsave("../output/functional_enrichment/Upregulated_shared_genes_WithErrorBars.png", shared_up_error, width = 15, height = 14)
```
...
Plot upregulated GO slim information
```{r}
...
I want to make a plot similar to one that Hollie made (see [here](https://github.com/hputnam/Poc_RAPID/blob/main/RAnalysis/figures/DEG_LFC_Larvae_Temperature.jpg)). It plots LFC on the x-axis, GO term on the y-axis, and p-value as color. I am going to make it for the 4 up/down regulated genes of interest. 

Read in High v Control DEG and GO information 
```{r}
h_v_c_deg_go <- read.csv("../output/functional_enrichment/High_v_Control_GO.csv")
h_v_c_deg_go$comparison <- "High_v_Control"
```

Read in High v Mid DEG and GO information 
```{r}
h_v_m_deg_go <- read.csv("../output/functional_enrichment/High_v_Mid_GO.csv")
h_v_m_deg_go$comparison <- "High_v_Mid"
```
...
# Select genes of interest for High v Control 
h_v_c_down_filt <- h_v_c_deg_go %>% 
  filter(gene_id %in% down) %>%
  na.omit() %>%
  dplyr::select(gene_id, log2FoldChange, padj, GO.ID, numDEInCat, term, ontology, comparison)

length(unique(h_v_c_down_filt$gene_id))

# Select genes of interest for High v Mid 
h_v_m_down_filt <- h_v_m_deg_go %>% 
  filter(gene_id %in% down) %>%
  na.omit() %>%
  dplyr::select(gene_id, log2FoldChange, padj, GO.ID, numDEInCat, term, ontology, comparison)

length(unique(h_v_m_down_filt$gene_id))
```

There are some repeated terms (as some of the terms are assigned to multiple genes). This is giving me some plotting issues. I'm going to remove repeated terms and only keep the one with the lowest padj value for each term. 
```{r}
# Reduce the High and Control df 
h_v_c_down_filt <- h_v_c_down_filt %>%
  group_by(term) %>%
...
# Reduce the High and Mid df 
h_v_m_down_filt <- h_v_m_down_filt %>%
  group_by(term) %>%
  filter(padj == min(padj))
...
Plot so that LFC is on x-axis, terms are on the y-axis, color is padj value for each comparison. 
```{r}
ggplot(h_v_c_down_filt, aes(x = term, y = log2FoldChange, colour = padj)) +
  ylim(-6, 6) +
  geom_point(size = 2) +
  geom_segment(aes(x = term, xend = term, y = 0, yend = log2FoldChange)) +
...
ggplot(h_v_m_down_filt, aes(x = term, y = log2FoldChange, colour = padj)) +
  ylim(-6, 6) +
  geom_point(size = 2) +
  geom_segment(aes(x = term, xend = term, y = 0, yend = log2FoldChange)) +
  #facet_wrap(~ ontology, ncol = 1, scales = "free_y") +
...
Still not ordering by LFC...not sure why?
...
# Select genes of interest for High v Control 
h_v_c_up_filt <- h_v_c_deg_go %>% 
  filter(gene_id %in% up) %>%
  na.omit() %>%
  dplyr::select(gene_id, log2FoldChange, padj, GO.ID, numDEInCat, term, ontology, comparison)

length(unique(h_v_c_up_filt$gene_id))

# Select genes of interest for High v Mid 
h_v_m_up_filt <- h_v_m_deg_go %>% 
  filter(gene_id %in% up) %>%
  na.omit() %>%
  dplyr::select(gene_id, log2FoldChange, padj, GO.ID, numDEInCat, term, ontology, comparison)

length(unique(h_v_m_up_filt$gene_id))
```

There are some repeated terms (as some of the terms are assigned to multiple genes). This is giving me some plotting issues. I'm going to remove repeated terms and only keep the one with the lowest padj value for each term. 
```{r}
# Reduce the High and Control df 
h_v_c_up_filt <- h_v_c_up_filt %>%
  group_by(term) %>%
  filter(padj == min(padj)) 
...
Plot so that LFC is on x-axis, terms are on the y-axis, color is padj value for each comparison. 
```{r}
ggplot(h_v_c_up_filt, aes(x = term, y = log2FoldChange, colour = padj)) +
  ylim(-6, 6) +
  geom_point(size = 2) +
  geom_segment(aes(x = term, xend = term, y = 0, yend = log2FoldChange)) +
  #facet_wrap(~ ontology, ncol = 1, scales = "free_y") +
...
ggplot(h_v_m_up_filt, aes(x = term, y = log2FoldChange, colour = padj)) +
  ylim(-6, 6) +
  geom_point(size = 2) +
  geom_segment(aes(x = term, xend = term, y = 0, yend = log2FoldChange)) +
  #facet_wrap(~ ontology, ncol = 1, scales = "free_y") +
...
Combine up and down terms for both comparisons and plot
```{r}
test <- rbind(h_v_c_up_filt, h_v_c_down_filt)

ggplot(test, aes(x = term, y = log2FoldChange, colour = padj)) +
  ylim(-6, 6) +
  geom_point(size = 2) +
  geom_segment(aes(x = term, xend = term, y = 0, yend = log2FoldChange)) +
  #facet_wrap(~ ontology, ncol = 1, scales = "free_y") +
...
Plot attempts 
```{r}
ggplot(h_v_c_down_filt, aes(x=term, y=log2FoldChange, colour=padj, group = gene_id)) +
  ylim(-16,16)+
  geom_point(size=2)+
...
# Create a new variable combining term and gene id
h_v_c_down_filt$term_padj <- paste(h_v_c_down_filt$term, "(",h_v_c_down_filt$gene_id,")")
h_v_c_down_filt$term_padj <- as.character(h_v_c_down_filt$term_padj)

# Plot with the new variable on the y-axis
ggplot(h_v_c_down_filt, aes(x = log2FoldChange, y = term_padj, colour = padj)) +
  #ylim(-16, 16) +
  geom_point(size = 2) +
  geom_segment(aes(x = term_padj, xend = term_padj, y = 0, yend = log2FoldChange)) +
  coord_flip() +
  theme_classic() +
  theme(legend.position = c(0.8, 0.45),
        axis.title.y = element_blank(),
...
Combine up and down terms for both comparisons and plot
```{r}
test <- rbind(h_v_c_up_filt, h_v_c_up_filt)
