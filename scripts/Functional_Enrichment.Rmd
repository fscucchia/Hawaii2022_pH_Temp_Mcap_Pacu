...
Pocillopora_acuta_HIv2___TS.g21597.t2). On the other hand, there are 164 and 120 genes with significant GO terms between H v C and H v M corals, respectively. 
...
- Pocillopora_acuta_HIv2___RNAseq.22237_t (-7.667258)
- 
...
I want to make a volcano plot for High v Control and High v Mid that labels the 4 upregulated and 4 downregulated genes of interest. 

Read in High v Control DEG information. Don't include the GO information here, as not all of the DEGs had go information. 
```{r}
h_v_c_deg <- read.csv("../output/DEG/High_v_Control_DEGs.csv")
length(unique(h_v_c_deg$X))
```

Add a column to the df that says if the gene is up or down regulated
```{r}
# Add a column to the data frame to specify if they are UP- or DOWN- regulated (log2fc respectively positive or negative)
h_v_c_deg$diffexpressed <- "NO"

# if log2Foldchange > 1 and adj pvalue < 0.05, set as "UP"
h_v_c_deg$diffexpressed[h_v_c_deg$log2FoldChange > 1.0 & h_v_c_deg$padj < 0.05] <- "UP"

# if log2Foldchange < -1 and adj pvalue < 0.05, set as "UP"
h_v_c_deg$diffexpressed[h_v_c_deg$log2FoldChange < -1.0 & h_v_c_deg$padj < 0.05] <- "DOWN"
```

Add a column that says which genes to label on plot 
```{r}
gene_interest <- list("Pocillopora_acuta_HIv2___RNAseq.g24121.t1", "Pocillopora_acuta_HIv2___RNAseq.g13974.t1", "Pocillopora_acuta_HIv2___TS.g25049.t1", "Pocillopora_acuta_HIv2___TS.g8000.t1", "Pocillopora_acuta_HIv2___RNAseq.g22884.t1", "Pocillopora_acuta_HIv2___TS.g23786.t1", "Pocillopora_acuta_HIv2___TS.g26760.t1", "Pocillopora_acuta_HIv2___RNAseq.g19477.t1")

h_v_c_deg$delabel <- ifelse(h_v_c_deg$X %in% gene_interest, h_v_c_deg$X, NA)

# Remove extra gene name information. This will help with the label on the volcano plot
h_v_c_deg$delabel <- sub("Pocillopora_acuta_HIv2___RNAseq.", "", h_v_c_deg$delabel)
h_v_c_deg$delabel <- sub("Pocillopora_acuta_HIv2___TS.", "", h_v_c_deg$delabel)
```


Make a volcano plot where LFC is on the x-axis and adj pvalue is on the y-axis for the high v control comparison.
```{r}
ggplot(h_v_c_deg, aes(x = log2FoldChange, y = -log10(padj), color = diffexpressed, label = delabel)) + 
  geom_point(size = 2) +
...
###ADD LABEL REPEL 
  ...
  labs(color = 'High v. Control', #legend_title, 
