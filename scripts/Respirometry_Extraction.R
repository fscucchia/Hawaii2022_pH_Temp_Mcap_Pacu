## Title: Mcap 2022 respiration & photosynthesis
## Author: Jill Ashey 
## Date created: 20230605

#### This script will calculate photosynthesis and respiration rates for M. capitata larvae from Hawaii 2022 pH and temperature experiment
...
# Set path of all resp files 
path.p <- "Raw_data/Respirometry/Runs" # location of files 

# Bring in file names 
file.names<-basename(list.files(path = path.p, pattern = "csv$", recursive = TRUE)) #list all csv file names in the folder and subfolders

# Generate respiration dataframes
...
Sample.Info <- read.csv("Raw_data/Respirometry/Resp_Sample_Info.csv") # info on treatments, runs, # of individuals, etc
...
# Load in respiration start times as a list 
#################### NOTE: START TIME INFO MAY CHANGE - NEED TO DISCUSS W/ ARIANA #################### 
starttimes<-read.csv("Raw_data/Respirometry/starttimes.csv") #read in file of starting R and P phase times
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("output/Respiration/RespirationPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
# Remove NAs from df 
Resp.R <- na.omit(Resp.R)

# Save respiration dataframe 
write.csv(Resp.R, "output/Respiration/Respiration_rates_Mcap2022.csv")

# Read file back in so slopes don't have to be generated every time 
Resp.Rates <- read.csv("output/Respiration/Respiration_rates_Mcap2022.csv")

# Remove empty column and format run column 
Resp.Rates <- Resp.Rates %>%
  select(-c(X)) %>%
  mutate(Run = str_sub(Run, 4, -1))
Resp.Rates$Run <- as.integer(Resp.Rates$Run)
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("output/Respiration/PhotosynthesisPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output files
    plot(model)
    dev.off()
...
# Save respiration dataframe 
write.csv(Photo.R, "output/Respiration/Photosynthesis_rates_Mcap2022.csv")

# Read file back in so slopes don't have to be generated every time 
Photo.Rates <- read.csv("output/Respiration/Photosynthesis_rates_Mcap2022.csv")

# Remove empty column and format run column 
Photo.Rates <- Photo.Rates %>%
  select(-c(X)) %>%
  mutate(Run = str_sub(Run, 4, -1))
Photo.Rates$Run <- as.integer(Photo.Rates$Run)

# Merge P and R files with sample info 
oxygen <- full_join(Sample.Info, Resp.Rates) # add respiration rates 
...
#################### MAY NEED TO REDO - LOOK OVER PDF OUTPUTS W/ ARIANA 
...
# Merge blank data from R and P to `oxygen` 
oxygen <- full_join(oxygen, resp.blnk) #add R blanks to master
oxygen <- full_join(oxygen, photo.blnk) #add P blanks to master

# Add blank values into dataframe and add avg blank values to replace any NA values
oxygen<-oxygen%>%
...
# Normalize to biologically relevant measure. Here, we are normalizing to number of larvae 
...
# Save as csv file 
write.csv(oxygen.bio, "output/Respiration/oxygen_P_R_calc_Mcap2022.csv")
